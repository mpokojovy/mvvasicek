function plot_comparison(time_grid_historic, time_grid_benchmark, ...
    ...
    title('Our ML', 'interpreter', 'latex', 'FontSize', 18);
    ...
    US_pred = plot(time_grid_benchmark, mean(squeeze(projection_ML(1, :, :)), 2), ...
                   'Color', [0,      0.4470, 0.7410], 'LineStyle', '--');
    EU_pred = plot(time_grid_benchmark, mean(squeeze(projection_ML(2, :, :)), 2), ...
                   'Color', [0.8500, 0.3250, 0.0980], 'LineStyle', '--');

    plot(time_grid_benchmark, quantile(squeeze(projection_ML(1, :, :)), 0.5*(1.0 - conf_level), 2), 'k:');
    plot(time_grid_benchmark, quantile(squeeze(projection_ML(1, :, :)), 1.0 - 0.5*(1.0 - conf_level), 2), 'k:');
    
    plot(time_grid_benchmark, quantile(squeeze(projection_ML(2, :, :)), 0.5*(1.0 - conf_level), 2), 'k:');
    plot(time_grid_benchmark, quantile(squeeze(projection_ML(2, :, :)), 1.0 - 0.5*(1.0 - conf_level), 2), 'k:');
    ...
    plot(time_grid_benchmark, quantile(squeeze(projection_OLS(1, :, :)), 0.5*(1.0 - conf_level), 2), 'k:');
    plot(time_grid_benchmark, quantile(squeeze(projection_OLS(1, :, :)), 1.0 - 0.5*(1.0 - conf_level), 2), 'k:');
    
    plot(time_grid_benchmark, quantile(squeeze(projection_OLS(2, :, :)), 0.5*(1.0 - conf_level), 2), 'k:');
    plot(time_grid_benchmark, quantile(squeeze(projection_OLS(2, :, :)), 1.0 - 0.5*(1.0 - conf_level), 2), 'k:');
